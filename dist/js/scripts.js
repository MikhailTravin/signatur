...
function rangeInit() {
  ...
  if (ratingCalc) {
    ...
    const priceInput = ratingCalc.closest('.filter-apartments__rating').querySelector('.filter-apartments__price input');
  ...
  const ratingCalcSquare = document.querySelector('.filter-square-range');

  if (ratingCalcSquare) {
    const rangeStart = Number(ratingCalcSquare.dataset.start);
    const rangeMin = Number(ratingCalcSquare.dataset.min);
    const rangeMax = Number(ratingCalcSquare.dataset.max);

    noUiSlider.create(ratingCalcSquare, {
      start: rangeStart || 90,
      connect: 'lower',
      range: {
        'min': rangeMin || 70,
    ...
    const priceInputSquare = ratingCalcSquare.closest('.filter-apartments__rating').querySelector('.filter-apartments__price.filter-square input');

    if (priceInputSquare) {
      ratingCalcSquare.noUiSlider.on('update', function (values, handle) {
        const value = parseFloat(values[handle]);
        const amount = Math.round(value);
        priceInputSquare.value = amount.toLocaleString('ru-RU');
      });

      const initialValue = 90;
      const initialAmount = Math.round(initialValue);
      priceInputSquare.value = initialAmount.toLocaleString('ru-RU');
    }
  }
